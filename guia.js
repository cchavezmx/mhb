...
(function () {
  ...
  function pintarNav() {
    ...
    if (ant) {
      ...
      if (i > 0) {
        ant.href = lista[i - 1].f;
        ant.removeAttribute("aria-disabled");
      } else {
        ant.removeAttribute("href");
    ...
    if (sig) {
      ...
      if (i < total - 1) {
        sig.href = lista[i + 1].f;
        sig.removeAttribute("aria-disabled");
      } else {
  ...
  function elegirVoz(codigo) {
    ...
    for (var r = 0; r < regs.length; r++) {
      var reg = regs[r];
      var cand = vs.filter(function (v) {
        return v.lang.replace("_", "-").toLowerCase().indexOf(reg.toLowerCase()) === 0;
      });
      if (!cand.length) continue;
      var h = cand.find(function (v) {
        return HOMBRES.some(function (n) {
          return v.name.toLowerCase().indexOf(n) >= 0;
        });
      });
      if (h) return h;
      var neutra = cand.find(function (v) {
        return !MUJERES.some(function (n) {
          return v.name.toLowerCase().indexOf(n) >= 0;
        });
      });
  ...
  function hayVoz(codigo) {
    ...
    return vs.some(function (v) {
      return v.lang.slice(0, 2) === codigo.slice(0, 2);
    });
  }
  ...
  function escuchar() {
    ...
    if (!window.speechSynthesis || !hayVoz(g.voz)) {
      if ($("ui-aviso")) {
        $("ui-aviso").textContent = g.ui.aviso;
